...
- 采用"非破坏性扩展"策略：不修改原 MetricsReporter 接口，新增可选扩展接口 PipelineMetricsReporter 来承接管道级指标
...
| 指标名称 | 类型 | 描述 | 标签 |
|---------|------|------|------|
| `batchflow_pipeline_process_duration_seconds` | Histogram | 管道处理耗时 | `database`, `instance_id`, `status` |
| `batchflow_pipeline_dequeue_latency_seconds` | Histogram | 管道出队延迟 | `database`, `instance_id` |
| `batchflow_pipeline_dropped_total` | Counter | 管道丢弃计数 | `database`, `instance_id`, `reason` |

注意：
- 所有指标都包含 instance_id 标签，用于区分多个 BatchFlow 实例
- 集成测试中使用测试名称作为 instance_id
- 生产环境中使用业务标识作为 instance_id（如 "order_writer"）
...
- 新增/优化的"流水线阶段（核心阶段指标）"分组（单位为秒，除丢弃错误速率为 req/s）：
  - 入队延迟 (p50/p95/p99)
  - 攒批耗时 (p50/p95/p99)
  - 执行耗时 (p50/p95/p99)（按实例）
  - 流水线处理耗时 (p50/p95/p99)（按状态）
  - 流水线出队等待 (p50/p95/p99)
  - 流水线丢弃错误速率 (by reason)
- 执行耗时面板聚合修正为 sum by (le, instance_id)，避免多余分组导致曲线缺失
...
- 面板无曲线：确认对应直方图 bucket 是否产生样本；执行耗时面板需确保 instance_id 标签存在且与仪表盘变量匹配
...
- 修复并细化面板：执行耗时(按实例)聚合、单位补齐、布局对齐
- 标签体系升级：instance_id 替代 test_name，支持多实例场景
